...
namespace hse {
    ...
    std::expected<NetworkManager::HttpConnection, NetworkError> NetworkManager::ParseUrl(const std::string& url) const noexcept {
        ...
        return HttpConnection{
            .host = std::wstring(hostBuffer.data()),
            .path = std::wstring(pathBuffer.data()),
            .port = urlComp.nPort
        };
    }

    std::expected<WinHttpSession, NetworkError> NetworkManager::CreateSession(const HttpConnection& connection) const noexcept {
        WinHttpSession session;

        session.set_session(WinHttpOpen(
            USER_AGENT.data(),
        ...
        session.set_connection(WinHttpConnect(
            session.session(),
    ...
    std::expected<void, NetworkError> NetworkManager::SendRequest(HINTERNET request, int connectTimeoutMs, int receiveTimeoutMs) const noexcept {
    ...
    std::expected<DownloadResult, NetworkError> NetworkManager::DownloadFile(const DownloadConfig& config) noexcept {
        ...
        struct FileGuard {
            HANDLE handle;
            ~FileGuard() { if (handle != INVALID_HANDLE_VALUE) CloseHandle(handle); }
        } fileGuard{ hFile };
        ...
        while (WinHttpQueryDataAvailable(session->request(), &dwSize) && dwSize > 0) {
            ...
            if (!WriteFile(hFile, buffer.data(), dwDownloaded, &dwWritten, nullptr) ||
