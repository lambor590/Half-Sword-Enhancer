...
WorldActionsSection::WorldActionsSection(ModContext& ctx) : CollapsibleSection(ctx, "Actions") {
    Function("Toggle Slow Motion")
        .WithKey(&cfg.sloMoKey)
        .WithParams(
            {Parameter("speed", "Speed", &cfg.slowMotionSpeed, 0.01f, 0.99f, "The speed of the game when is enabled")}
        )
        .Action(
            [this]() {
