                    ...
                    GameHook::QueueAction([this]() {
                        ...
                        ActorUtils::ForEachObjectOfType<SDK::ACSBloodSimActor>(
                            ...
                            [this, clearColor, defaultVertexColor](SDK::ACSBloodSimActor* sim) {
                                if (sim->CurrentWrite)
                                    SDK::UKismetRenderingLibrary::ClearRenderTarget2D(
                                        world, sim->CurrentWrite, clearColor
                                    );
                                if (sim->CurrentRead)
                                    SDK::UKismetRenderingLibrary::ClearRenderTarget2D(
                                        world, sim->CurrentRead, clearColor
                                    );
                                if (sim->BoundMesh)
                                    SDK::UMeshVertexPainterKismetLibrary::PaintVerticesSingleColor(
                        ...
                        ActorUtils::ForEachComponentOfType<SDK::UDecalComponent>(
                            world, [](SDK::UDecalComponent* decal) { decal->K2_DestroyComponent(decal); }
                        );
                        ActorUtils::ForEachObjectOfType<SDK::ADecalActor>(world, [](SDK::ADecalActor* decal) {
                            decal->K2_DestroyActor();
                        });
                        ActorUtils::ForEachObjectOfType<SDK::ARunningBlood_BP_C>(
                            world, [](SDK::ARunningBlood_BP_C* blood) { blood->K2_DestroyActor(); }
                        );
