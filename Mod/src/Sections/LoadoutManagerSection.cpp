...
void LoadoutManagerSection::GenerateWeaponForSlot(int slotIndex) {
    ...
    GameHook::QueueAction([slotIndex, tier, type, specificType, generateGreatsword](const RuntimeContextSnapshot& runtime) {
        ...
        auto& slot = LoadoutPresetData::GetWeaponSlot(weapons, slotIndex);
        EquipmentApplication::WriteWeaponPassportToSlot(passport, slot);
    });
...
void LoadoutManagerSection::ImportWeaponPreset(int slotIndex) {
    ...
    GameHook::QueueAction([slotIndex, data = std::move(data)](const RuntimeContextSnapshot& runtime) {
        ...
        auto passport = data.passport;
        EquipmentApplication::ResolveWeaponPassportClasses(passport, data.classPaths);
        EquipmentApplication::WriteWeaponPassportToSlot(passport, slot);
    });
