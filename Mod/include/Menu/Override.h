...
constexpr OverrideDescriptor OverrideField(
    const char* name, IntOverride& ovr, int defaultVal = 0, int minVal = 0, int maxVal = 0
);
...
constexpr OverrideDescriptor OverrideField(
    const char* name, RuntimeOverride& ovr, double defaultVal, double minVal, double maxVal
) {
    return {name, &ovr.enabled, &ovr.value, OverrideFieldType::Double, defaultVal, minVal, maxVal};
}

constexpr OverrideDescriptor OverrideField(const char* name, IntOverride& ovr, int defaultVal, int minVal, int maxVal) {
    return {
        name,
        &ovr.enabled,
        &ovr.value,
...
constexpr OverrideDescriptor OverrideField(const char* name, BoolOverride& ovr, bool defaultVal) {
    return {name, &ovr.enabled, &ovr.value, OverrideFieldType::Bool, defaultVal ? 1.0 : 0.0, 0.0, 0.0};
}
