...
inline SDK::FStr_Passport_Weapon1 FromWeaponParts(SDK::FStr_WeaponParts& parts) {
    ...
    for (auto it = begin(matMap); it != end(matMap); ++it) {
        switch (it->Key()) {
            case SDK::Enum_Weapon_Material_Type::NewEnumerator0:
                p.MaterialMetalSteel_37_AB7A28C94B176CF81A6C8BA34AC57C36 = it->Value(); break;
            case SDK::Enum_Weapon_Material_Type::NewEnumerator1:
                p.MaterialMetalColored_39_DC2EAC244758A8D82855CC940784A1D2 = it->Value(); break;
            case SDK::Enum_Weapon_Material_Type::NewEnumerator2:
                p.MaterialWeood_41_E0B3C8DB48943B878AEFA3AB01E7B99A = it->Value(); break;
            case SDK::Enum_Weapon_Material_Type::NewEnumerator3:
                p.MaterialLeather_43_41D1114148FDB4FE4DACC8A2F4CA9FEB = it->Value(); break;
            default: break;
        }
    }
    ...
    for (auto it = begin(colorMap); it != end(colorMap); ++it) {
        switch (it->Key()) {
            case SDK::Enum_Weapon_Material_Type::NewEnumerator2:
                p.ColorWood_46_F3AE05AD4495EBCD1D354C8025D7C743 = it->Value(); break;
            case SDK::Enum_Weapon_Material_Type::NewEnumerator3:
                p.ColorLeather_48_DC45F07E4C0C3280278212A7158EE638 = it->Value(); break;
            default: break;
        }
    }
