...
enum class PropType : uint8_t {
    Float, Double, Int, Bool, Byte, Enum,
    LinearColor, Color, Vector, Rotator,
    Unsupported
};
...
[[nodiscard]] inline std::string CleanPropertyName(const std::string& raw) {
    ...
    if (cleaned.size() > 9 && cleaned[cleaned.size() - 9] == '_') {
        ...
        for (size_t i = cleaned.size() - 8; i < cleaned.size(); ++i) {
            if (!IsHexChar(cleaned[i])) { allHex = false; break; }
        }
    ...
    if (cleaned.size() > 1) {
        ...
        while (i > 0 && std::isdigit(static_cast<unsigned char>(cleaned[i - 1]))) --i;
        if (i > 0 && i < cleaned.size() && cleaned[i - 1] == '_')
            cleaned.erase(i - 1);
    }
...
[[nodiscard]] inline PropType ClassifyProperty(SDK::FProperty* prop, SDK::UEnum*& outEnum) {
    ...
    if (castFlags & SDK::EClassCastFlags::BoolProperty)
        return PropType::Bool;
    if (castFlags & SDK::EClassCastFlags::FloatProperty)
        return PropType::Float;
    if (castFlags & SDK::EClassCastFlags::DoubleProperty)
        return PropType::Double;
    if (castFlags & SDK::EClassCastFlags::IntProperty)
        return PropType::Int;

    if (castFlags & SDK::EClassCastFlags::ByteProperty) {
        auto* bp = static_cast<SDK::FByteProperty*>(prop);
        if (bp->Enum) { outEnum = bp->Enum; return PropType::Enum; }
...
[[nodiscard]] inline std::vector<std::string> BuildEnumNames(SDK::UEnum* enumPtr) {
    ...
    SDK::UUserDefinedEnum* udEnum = enumPtr->IsA(SDK::UUserDefinedEnum::StaticClass())
        ? static_cast<SDK::UUserDefinedEnum*>(enumPtr) : nullptr;
    ...
    for (int32_t i = 0; i < enumNames.Num(); ++i) {
        ...
        if (fullName.size() >= 4 && fullName.compare(fullName.size() - 4, 4, "_MAX") == 0)
            continue;
...
inline bool RenderPropertyWidget(const PropertyInfo& prop, void* objectBase) {
    ...
    case PropType::Float: {
        ...
        changed = ImGui::DragFloat(prop.displayName.c_str(), reinterpret_cast<float*>(valuePtr), 0.01f, 0.0f, 0.0f, "%.4f");
        break;
    }
    case PropType::Double: {
        ImGui::SetNextItemWidth(kScalarWidth);
        changed = ImGui::DragScalar(prop.displayName.c_str(), ImGuiDataType_Double, reinterpret_cast<double*>(valuePtr), 0.01f, nullptr, nullptr, "%.4f");
        break;
    }
    case PropType::Int: {
        ImGui::SetNextItemWidth(kScalarWidth);
    ...
    case PropType::Enum: {
        int intVal = (prop.elementSize <= 1) ? static_cast<int>(*valuePtr)
                                             : *reinterpret_cast<int32_t*>(valuePtr);

        if (!prop.enumNames.empty()) {
            const char* preview = (intVal >= 0 && intVal < static_cast<int>(prop.enumNames.size()))
                ? prop.enumNames[intVal].c_str() : "Unknown";
            ...
            if (ImGui::BeginCombo(prop.displayName.c_str(), preview)) {
                for (int i = 0; i < static_cast<int>(prop.enumNames.size()); ++i) {
                    if (ImGui::Selectable(prop.enumNames[i].c_str(), i == intVal)) {
                        if (prop.elementSize <= 1) *valuePtr = static_cast<uint8_t>(i);
                        else *reinterpret_cast<int32_t*>(valuePtr) = i;
                        changed = true;
                    }
            ...
            ImGui::SetNextItemWidth(kScalarWidth);
            if (ImGui::DragInt(prop.displayName.c_str(), &intVal, 1.0f, 0, 255)) {
                if (prop.elementSize <= 1) *valuePtr = static_cast<uint8_t>(intVal);
                else *reinterpret_cast<int32_t*>(valuePtr) = intVal;
                changed = true;
            }
        }
        break;
    ...
    case PropType::Color: {
        ...
        float col[4] = {
            bytes[2] / 255.0f, bytes[1] / 255.0f,
            bytes[0] / 255.0f, bytes[3] / 255.0f
        };
        ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x * 0.75f);
